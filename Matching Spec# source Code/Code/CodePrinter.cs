...
namespace System.Compiler{
  ...
  public abstract class CodePrinter {
    ...
    public static string NodeToString(Node node)
    {
        ...
        if(type.IsSubclassOf(typeof(Quantifier)))
        {
            sb.Append(quantifier2str((Quantifier)node));
        }
        else if (type.IsSubclassOf(typeof(Expression)))
        {
            sb.Append(expression2str((Expression)node));
        }
...
//ll
  public static string expression2str(Expression expr, bool top_level) {
...
private static string oldExpression2str(NodeType nt)
{
    StringBuilder sb = new StringBuilder();
    switch (nt)
    {
    ...
    private static string quantifiertype2str(NodeType qt)
    {
        StringBuilder sb = new StringBuilder();
        switch (qt)
        {
            ...
            case NodeType.Forall:
                sb.Append("forall");break;    
            case NodeType.Sum:
                sb.Append("sum"); break;
            case NodeType.Product:
                sb.Append("product");break;
            case NodeType.Max:
                sb.Append("max");break;
            case NodeType.Min:
                sb.Append("min");break;
    ...
    private static string typeexp2str(TypeExpression te)
    {
        StringBuilder sb = new StringBuilder();
        switch (te.Expression.ToString())
        {
            case "Int32":
                sb.Append("int");break;     
            case "Boolean":
                sb.Append("bool");break;
            default:
                sb.Append(te.Expression.ToString());break;
        }
        return sb.ToString();
    }
    private static string mode2str(ComprehensionBindingMode cbm)
    {
        StringBuilder sb=new StringBuilder();
        switch (cbm)
        {
            case ComprehensionBindingMode.In:
                sb.Append("in");
                break;
            case ComprehensionBindingMode.Gets:
                sb.Append("gets");
                break;
        }
        return sb.ToString();
    }
    private static string scope2str(Expression scope)
    {
        StringBuilder sb = new StringBuilder();
        //CollectionEnumerator ce = (CollectionEnumerator)scope;
        UnaryExpression ue = (UnaryExpression)scope;
        BinaryExpression be = (BinaryExpression)(ue.Operand);

        sb.Append("(");
        sb.Append(expression2str(be.Operand1, false));
        sb.Append(":");

        sb.Append(expression2str(be.Operand2,false));
        sb.Append(")");
        return sb.ToString();
    }
    ...
    private static string unaryexpr2str(UnaryExpression ue) {
      ...
      return expression2str(ue.Operand, false);
    }

    public static string op2str(NodeType type) {
      switch(type) {
